-- Optional free-text answer collected during event registration.

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS referral_source text;

-- Recover answers submitted through Auth metadata before the profile row was ready.
UPDATE public.profiles AS profile
SET referral_source = NULLIF(auth_user.raw_user_meta_data->>'referral_source', '')
FROM auth.users AS auth_user
WHERE profile.id = auth_user.id
  AND NULLIF(profile.referral_source, '') IS NULL
  AND NULLIF(auth_user.raw_user_meta_data->>'referral_source', '') IS NOT NULL;
...
BEGIN
  ...
  INSERT INTO public.profiles (
    ...
    linkedin, website, preferred_model, country, city, organization, age_range, referral_source
  )
  VALUES (
    NEW.id,
    NEW.email,
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    referral_source = COALESCE(NULLIF(public.profiles.referral_source, ''), EXCLUDED.referral_source);
